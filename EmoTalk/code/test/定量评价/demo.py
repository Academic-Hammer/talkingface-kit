...
#python demo.py --wav_path ./test_audio/Jae-in.wav
...
def test(args):
    ...
    eye1 = np.array([
        0.36537236, 0.950235724, 0.95593375, 0.916715622, 0.367256105,
        0.119113259, 0.025357503
    ])
    eye2 = np.array([
        0.234776169, 0.909951985, 0.944758058, 0.777862132, 0.191071674,
        0.235437036, 0.089163929
    ])
    eye3 = np.array([
        0.870040774, 0.949833691, 0.949418545, 0.695911646, 0.191071674,
        0.072576277, 0.007108896
    ])
    eye4 = np.array([
        0.000307991, 0.556701422, 0.952656746, 0.942345619, 0.425857186,
        0.148335218, 0.017659493
    ])
    model = EmoTalk(args)
    ...
    speech_array, sampling_rate = librosa.load(os.path.join(wav_path),
                                               sr=16000)
    ...
    if args.post_processing:
        ...
        while i < output.shape[0] - 7:
            ...
            i = i + time1
        np.save(os.path.join(result_path, "{}.npy".format(file_name)),
                output)  # with postprocessing (smoothing and blinking)
    else:
        np.save(os.path.join(result_path, "{}.npy".format(file_name)),
...
def render_video(args):
    ...
    cmd = '{} -t 64 -b {} -P {} -- "{}" "{}" '.format(blender_path, blend_path,
                                                      python_path, result_path,
                                                      wav_name)
    cmd = shlex.split(cmd)
    ...
    # cmd = 'ffmpeg -r 30 -i "{}" -i "{}" -pix_fmt yuv420p -s 512x768 "{}" -y'.format(
    #     image_temp, args.wav_path, output_path)
...
def main():
    ...
    parser.add_argument("--wav_path",
                        type=str,
                        default="./audio/angry1.wav",
                        help='path of the test data')
    parser.add_argument("--bs_dim",
                        type=int,
                        default=52,
                        help='number of blendshapes:52')
    ...
    parser.add_argument("--post_processing",
                        type=bool,
                        default=True,
                        help='whether to use post processing')
    parser.add_argument("--blender_path",
                        type=str,
                        default="./blender/blender",
                        help='path of blender')
