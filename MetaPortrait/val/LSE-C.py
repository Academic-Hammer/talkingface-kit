...
def calculate_lse_c(generated_keypoints):
    """
    计算 LSE-C（时间一致性）。
    :param generated_keypoints: 生成视频的关键点数组 [num_frames, 68, 2]
    :return: LSE-C 值
    """
    diff = np.diff(generated_keypoints, axis=0)  # 计算连续帧之间的差异
    lse_c = np.mean(np.linalg.norm(diff, axis=-1))  # 平均 L2 范数
    return lse_c
...
if __name__ == "__main__":
    ...
    # 创建命令行参数解析器
    parser = argparse.ArgumentParser(description='Calculate LSE-C (Temporal Consistency) for generated frames')
    ...
    parser.add_argument('--model_path', type=str, default='shape_predictor_68_face_landmarks.dat',
                        help='Path to the model for ID loss calculation')
    # 解析命令行参数
    args = parser.parse_args()

    # 加载 Dlib 的人脸检测器和68点模型
    ...
    # 计算 LSE-C
    lse_c = calculate_lse_c(generated_keypoints)

    # 输出 LSE-C 结果
    print(f"LSE-C (时间一致性): {lse_c}")
