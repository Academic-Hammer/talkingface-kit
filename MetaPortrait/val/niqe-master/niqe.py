...
if __name__ == "__main__":
    ...
    # 创建命令行参数解析器
    parser = argparse.ArgumentParser(description='Calculate NIQE score for images in a folder')
    ...
    # 获取文件夹中的所有图像文件
    image_files = [f for f in os.listdir(args.generated_folder) if f.endswith(('.png', '.jpg', '.jpeg'))]
    niqe_values = []
    NIQE_sum = 0
    count = 0
    ...
    for image_file in image_files:
        # 读取图像
        image_path = os.path.join(args.generated_folder, image_file)
        image = np.array(Image.open(image_path).convert('LA'))[:, :, 0]  # 转为灰度图像
