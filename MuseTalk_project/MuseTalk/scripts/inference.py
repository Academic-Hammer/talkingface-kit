...
def main(args):
    ...
    video_path = args.input_image #inference_config[task_id]["video_path"]
    audio_path = args.input_audio #inference_config[task_id]["audio_path"]
    bbox_shift = args.bbox_shift #inference_config[task_id].get("bbox_shift", args.bbox_shift)

    input_basename = os.path.basename(video_path).split('.')[0]
    audio_basename  = os.path.basename(audio_path).split('.')[0]
    output_basename = f"{input_basename}_{audio_basename}"
    result_img_save_path = os.path.join(args.output_dir, output_basename) # related to video & audio inputs
    crop_coord_save_path = os.path.join(result_img_save_path, input_basename+".pkl") # only related to video input
    os.makedirs(result_img_save_path,exist_ok =True)
    print(result_img_save_path)
    
    if args.output_vid_name is None:
        output_vid_name = os.path.join(args.output_dir, output_basename+".mp4")
    else:
        output_vid_name = os.path.join(args.output_dir, args.output_vid_name)
    ############################################## extract frames from source video ##############################################
    if get_file_type(video_path)=="video":
        save_dir_full = os.path.join(args.output_dir, input_basename)
