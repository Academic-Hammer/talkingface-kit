...
'''if __name__ == "__main__":
    def read_image_as_grayscale(image_path):
        ...
        return np.array(img)
    ref_bikes = read_image_as_grayscale('/home/lanlan/Talking-Face_PC-AVS-main/results/id_00010_pose_00473_audio_741400104/G_Pose_Driven_/G_Pose_Driven_0.jpg')  # 注意文件扩展名
    
    print('NIQE of the image is: %0.3f' % niqe(ref_bikes))'''
    
def read_image_as_grayscale(image_path):
    img = Image.open(image_path).convert('L')
    return np.array(img)

def evaluate_niqe_in_folder(folder_path):
    # 遍历文件夹中的所有文件
    niqe_values = []
    for filename in os.listdir(folder_path):
        # 检查文件扩展名，确保只处理图像文件（这里以.jpg为例）
        if filename.endswith(".jpg") or filename.endswith(".jpeg") or filename.endswith(".png"):
            # 构建完整的文件路径
            image_path = os.path.join(folder_path, filename)
            # 读取图像并转换为灰度图
            img_gray = read_image_as_grayscale(image_path)
            # 计算NIQE值
            niqe_value = niqe(img_gray)
            niqe_values.append(niqe_value)
            # 打印NIQE值和对应的图像文件名
            print(f'NIQE of the image "{filename}" is: %0.3f' % niqe_value)
    average_niqe = sum(niqe_values) / len(niqe_values) if niqe_values else 0
    print(f'Average NIQE of all images in the folder is: %0.3f' % average_niqe)

if __name__ == "__main__":
    folder_to_evaluate = '/home/lanlan/Talking-Face_PC-AVS-main/results/id_00010_pose_00473_audio_741400104/G_Pose_Driven_'
# 调用函数评估文件夹中的所有图像
    evaluate_niqe_in_folder(folder_to_evaluate)