...
def train(config, generator, discriminator, kp_detector, checkpoint, log_dir, dataset, device_ids, opt, writer):
    ...
    dataloader = DataLoader(dataset, batch_size=train_params['batch_size'], shuffle=True, num_workers=13,
                            drop_last=True)

    generator_full = getattr(MODEL, opt.GFM)(kp_detector, generator, discriminator, train_params, opt)
    discriminator_full = DiscriminatorFullModel(kp_detector, generator, discriminator, train_params)
    test_dataset = EvaluationDataset(dataroot='/root/autodl-tmp/vox-png',
                                     pairs_list='data/vox_evaluation.csv')
