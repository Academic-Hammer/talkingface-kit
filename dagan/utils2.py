...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("find_best_frame_video.log"),  # 将日志写入文件
...
def depth_gray(path):
    ...
    outputs = depth_decoder(depth_encoder(tensor_img))
    depth_source = outputs[("disp", 0)][0]
    depth_source = depth_source.permute(1, 2, 0).detach().cpu().numpy() * depth_source.permute(1, 2,
                                                                                               0).detach().cpu().numpy()
...
def aus(path):
    ...
    detector = Detector()
    # image_prediction = detector.detect_faces(path)
    out1 = detector.detect_faces('FID/source/0.jpg')
    out1.plot_aus(12, muscles={'all': "heatmap"}, gaze=None)
    plt.savefig('a.jpg')
    out2 = detector.detect_faces('FID/source/1.jpg')
...
def evaluate_PRMSE_AUCON():
    ...
    for im in tqdm(imgs):
        try:
            ...
            gen = os.path.join(path, 'generate_364_self', im)
        ...
        try:
            ...
            generate = Image.open(os.path.join(path, 'generate_364_self', im))
...
def drawKPline():
    os.makedirs('network_graph', exist_ok=True)
...
def find_best_frame_video():
    ...
    logger.info("开始执行 find_best_frame_video 函数")
    ...
    try:
        ...
        best_frame = []
        total = len(sources)
        logger.info(f"总共有 {total} 对 source 和 driving 需要处理")
    except Exception as e:
        logger.error(f"读取 CSV 文件时出错: {e}")
        return

    for i, (src, dri) in enumerate(tqdm(zip(sources, drivings), total=total, desc="Processing pairs")):
        try:
            ...
            if (i + 1) % 100 == 0:
                logger.info(f"已处理 {i + 1}/{total} 对 source 和 driving")
        except Exception as e:
            logger.error(f"处理第 {i + 1} 对 (source: {src}, driving: {dri}) 时出错: {e}")
            best_frame.append("Error")

    try:
        f = open('./data/vox_cross_id_evaluation_best_frame.csv', 'w', encoding='utf-8')
        csv_writer = csv.writer(f)
        ...
        csv_writer.writerows(content)
        f.close()
        logger.info("成功写入 vox_cross_id_evaluation_best_frame.csv 文件")
    except Exception as e:
...
def find_best_frame(source, driving, cpu=False):
    ...
    def normalize_kp(kp):
        ...
        try:
            ...
            logger.warning(f"计算 ConvexHull 时出错: {e}")
            area = 1  # 避免除以零
            kp[:, :2] = kp[:, :2] / area
        return kp
    ...
    for i, image in enumerate(driving):
        try:
            ...
            logger.warning(f"检测驱动图像第 {i + 1} 帧关键点时出错: {e}")
            continue
    return frame_num
...
if __name__ == '__main__':
    ...
    # create_cross_id_test_set('/root/autodl-tmp/vox-png/test')
    #  render('ppt_figure/0000021.jpg')
    #  depth_rgb('ppt_figure/293.jpg')
    # process_celeV('/data/fhongac/origDataset/CelebV')
    # compare()
    evaluate_PRMSE_AUCON()  # CUDA_VISIBLE_DEVICES=1 python utils.py
