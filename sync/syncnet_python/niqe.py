    ...
    for filename in os.listdir(folder_path):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
            ...
            try:
                ...
                niqe_score = calculate_niqe(image, crop_border=crop_border, params_path=params_path)
                niqe_scores.append(niqe_score)
#                 print(f"Processed {filename}: NIQE = {niqe_score}")
            except Exception as e:
                print(f"Error processing {filename}: {e}")

    if niqe_scores:
        average_niqe = sum(niqe_scores) / len(niqe_scores)
        print(f"Average NIQE score for folder '{folder_path}': {average_niqe}")
    else:
